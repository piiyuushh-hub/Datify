...
# Suppress sklearn warnings to keep stdout clean
...
def main():
    ...
    try:
        ...
        print(json.dumps({"error": f"Model failed to load: {str(e)}"}))
        sys.exit(1)

    # Hard business rule: inactive sim = no data usage
    if req.get('calls_made', 0) == 0 and req.get('sms_sent', 0) == 0:
        print(json.dumps({"predicted_data_used_gb": 0, "explanations": []}))
        sys.exit(0)
    ...
    try:
        ...
        weekend_ratio = req.get('weekend_usage_ratio', 0)
        ...
        print(json.dumps({
            "predicted_data_used_gb": predicted_gb,
